...
CSELSA (Content-Semantic ELSA) replaces ELSA's independent trainable factor
for every item with one shared feature projection. For item-feature matrix
``S`` and learned projection ``W``, it forms row-normalized factors
``A = normalize(S @ W)`` and trains the ELSA reconstruction
``X @ (A @ A.T - I)``. The optional projection bias is disabled by default.
Orthogonal projection initialization and row normalization keep the initial
factor geometry well scaled.

Because the model state contains only the projection, candidate features are
not copied into its ``state_dict``. Rebuilding the candidate catalog projects
new items with the already fitted ``W``. Unlike TEASER and TEASERGD, CSELSA
can also use a new item in source history immediately: both source and
candidate factors come from the current catalog's features. This is fully
inductive item cold start; no per-item encoder row is learned.

``train_item_indices`` optionally limits the interaction columns and feature
rows participating in the reconstruction objective while retaining every
supplied item in the initial candidate catalog. This is convenient for an item
cold-start split. Alternatively, pass already-sliced training interactions,
features, IDs, and metadata. ``max_output`` uses the same source-prefix
in-batch and sampled-negative strategy as ELSA.

At prediction time, call ``build_candidates`` with the catalog appropriate to
the validation or test period, then call ``align_source`` using the item IDs
describing the external matrix columns. Alignment targets the *current*
catalog, not the catalog seen during fitting. Projected catalog factors are
cached on the model device and invalidated atomically whenever candidates are
rebuilt, updated, or removed.

.. code-block:: python

   from compresso_recsys.evaluation import evaluate_recommender
   from compresso_recsys.models import CSELSAConfig, CSELSATrainer

   cs_elsa = CSELSATrainer(
       CSELSAConfig(
           latent_dim=800,
           device="cuda",
           batch_size=1024,
   ...
   cs_elsa.build_candidates(
       item_ids=split["test_item_ids"],
       item_features=test_item_embeddings,
       metadata=test_meta,
   )
   test_source = cs_elsa.align_source(
...
.. autoclass:: compresso_recsys.models.CSELSAConfig
   :members:

.. autoclass:: compresso_recsys.models.CSELSA
   :members:

.. autoclass:: compresso_recsys.models.CSELSATrainer
   :members:
